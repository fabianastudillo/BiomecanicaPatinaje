...
def smoothingkeypoints(frame_count,poses):
    ...
    for i in range(allpoints):
        ...
        if integer < 1:
            ...
            newposesx=smooth(posex,10,'hanning')
            newposesy=smooth(posey,10,'hanning')
...
def calcular_angulos(CAfram):
    Rfoot_ex=[]     #Pie externo derecho
    Lfoot_ex=[]     #Pie externo izquierdo
    Rfoot_in=[]     #Pie interno derecho
    Lfoot_in=[]     #Pie interno izquierdo
    Rpush_in=[]     #Empuje interno derecho
    Lpush_in=[]     #Empuje intero izquierdo
    Rpush_ex=[]     #Empuje externo derecho
    Lpush_ex=[]     #Empuje externo izquierdo
    Rankle=[]       #Tobillo derecho
    Lankle=[]       #Tobillo izquierdo
    Legs=[]         #Piernas
...
#     amp_front_max=max(feet_separation)*(dr/pix)
#     amp_front_min=min(feet_separation)*(dr/pix)
...
#     return [amp_front_max, amp_front_min, shoulder_distance_mean, Rmax_floor_sep, Lmax_floor_sep]
...
def write_csv1(video_name, angles, string):
    angles.insert(0, video_name)       
    # Agregamos nuevos datos al archivo CSV
    with open('Angulos/'+string+'.csv', 'a', newline='') as f:
        writer = csv.writer(f)
...
# carpeta='ArchivosJSON/Frontal_nombre/Agus/0'
...
ketpointssmooth=smoothingkeypoints(numframes,frames)
normalizar=norma(numframes,ketpointssmooth) 
newposition=position(numframes,normalizar) 
...
# write_csv1(name, Angulos[0],'Ang0f')
# write_csv1(name, Angulos[1],'Ang1f')
# write_csv1(name, Angulos[2],'Ang2f')
# write_csv1(name, Angulos[3],'Ang3f')
# write_csv1(name, Angulos[4],'Ang4f')
# write_csv1(name, Angulos[5],'Ang5f')
# write_csv1(name, Angulos[6],'Ang6f')
# write_csv1(name, Angulos[7],'Ang7f')
# write_csv1(name, Angulos[8],'Ang8f')
# write_csv1(name, Angulos[9],'Ang9f')
# write_csv1(name, Angulos[10],'Ang10f')
...
# gei_name = "GEI_frontal/"+name+".png"

# cv2.imwrite(gei_name, gei_mat)
# write_csv(AngMaxMin, VET)
